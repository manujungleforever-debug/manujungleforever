...
# Remove all team members and replace with Jordy and Gloria
start_first = content.find('<!-- Team Member')

# Find exactly where the next section starts
end_last = content.find('<h2 class=\'h2 r r-up\' style=\'text-align:center; margin-bottom:60px;\'>Destinations</h2>', start_first)
# The <hr> before it
if end_last != -1:
    end_last = content.rfind('<hr', start_first, end_last)
...
if start_first != -1 and end_last != -1:
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(content)
    print("Updated About Us team section!")
else:
    print("Could not find boundaries", start_first, end_last)
