...
# 1. Update auth.js to load the latest avatar from users.json dynamically
...
# 2. Grand, wide single-line header CSS
grand_header_css = """
/* Grand single-line header styles */
header {
    position: sticky !important;
    top: 0 !important;
    z-index: 200 !important;
    background: rgba(3, 8, 7, 0.94) !important;
...
.user-pill {
    ...
    align-items: center !important;
    gap: 10px !important;
    background: rgba(11, 38, 35, 0.7) !important;
...
.user-pill.superuser {
    ...
    box-shadow: 0 4px 18px rgba(201, 168, 76, 0.18) !important;
}
.user-pill img {
...
for d in admin_dirs:
    for fpath in glob.glob(f'{d}/*.html'):
        ...
        # Clean previous header style injection if exists
        ...
        # Replace inline logo height with 72px
        content = re.sub(r'style="height:\s*\d+px[^"]*"', 'style="height: 72px; width: auto;"', content)
        
        # Inject Grand CSS before </head>
        content = content.replace('</head>', f'<style>\n{grand_header_css}\n</style>\n</head>')
        
        # Ensure auth.js has cache bust v=5
        content = re.sub(r'src="js/auth\.js(\?v=\d+)?"', 'src="js/auth.js?v=5"', content)
        
        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Applied grand header styles in {fpath}")
